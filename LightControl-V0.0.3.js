let Version = "0.0.3" //Skript um Lichter in Helligkeit, Farbe und Farbtemp global zu steuern - Git: https://github.com/Pittini/iobroker-LightControl - Forum: https://forum.iobroker.net/topic/36578/vorlage-lightcontrol
...
LightGroups[1][5][3] = ["deconz.0.Lights.ccccccfffed68f5d.xy", "xy", "0.7329,0.2641", "0.4925,0.4148", "white"]; // Datenpunkt - Farbsystem - Standardfarbe(aktuell rot) - Farbe für warmweiss - Farbe für Tageslichtweiss 
...
LightGroups[1][6][3] = ["deconz.0.Lights.680ae2fffeae5254.xy", "xy", "0.7329,0.2641", "0.4925,0.4148", "white"];  // Datenpunkt - Farbsystem - Standardfarbe(aktuell rot) - Farbe für warmweiss - Farbe für Tageslichtweiss 
...
// ab 50 interne Werte - 50=Gruppenname, 51=AutoOn_Lux-Lockstate,
...
for (let x = 0; x < Groups.length; x++) {
    ...
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".RampOff.SwitchOutletsLast", initial: false, forceCreation: false, common: { read: true, write: true, name: "Sollen Devices am Rampenende geschaltet werden?", type: "boolean", def: false } };
    DpCount++;
...
function AutoOn_Lux() {
    ...
    for (let x = 0; x < Groups.length; x++) {
        if (Groups[x][24] && ActualLux <= Groups[x][25] && !Groups[x][0] && !Groups[x][51]) { //Wenn .AutoOn_Lux.Enabled true und eingestellter Luxwert unterschritten ist und Gruppe ist inaktiv
