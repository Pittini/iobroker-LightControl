const Version = "0.1.5" //vom 23.11.2020 - Skript um Lichter in Helligkeit, Farbe und Farbtemp global zu steuern - Git: https://github.com/Pittini/iobroker-LightControl - Forum: https://forum.iobroker.net/topic/36578/vorlage-lightcontrol
//To do:  Colorflow
log("starting LightControl V." + Version);

/* A C H T U N G ! Bei Altinstallationen vor V 0.1.4 bitte unbedingt Zeile 14 anpassen */
...
const Groups = [];
const GroupNames = ["Flur Eg", "Wohnzimmer", "Toilette", "Flur Og", "Bad", "Dach", "Schlafzimmer Carlo", "Kueche Unterbau", "Vitrine", "Hauseingang"]; //Initialisiert das Gruppenarray mit den Gruppennamen. In folge werden hier alle Gruppenstates eingelesen
...
// 13=".RampOn.Enabled", 14=".RampOff.Enabled", 15=".RampOn.Time", 16=".RampOff.Time", 17=".BlinkEnabled", 18=".Blink", 19=".OnOverride", 20= ".RampOff.SwitchOutletsLast", 21=".AutoOn_Motion.AutoOnBri", 22=".AutoOn_Motion.AutoOnColor", 
...
for (let x = 0; x < GroupNames.length; x++) {
    //Datenpunkte global
    States[DpCount] = { id: praefix + "." + x + ".Power", initial: false, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Gruppe aktiv?", type: "boolean", def: false } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".Bri", initial: 100, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Helligkeit", type: "number", unit: "%", def: 100 } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".Ct", initial: 100, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Farbtemperatur", type: "number", unit: "%", def: 100 } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".Color", initial: "#FFFFFF", forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Farbe", type: "string", def: "#FFFFFF" } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AutoOff_Timed.AutoOffTime", initial: 120, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Zeit nach der die Gruppe abgeschaltet wird", type: "number", unit: "s", def: 120 } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AutoOff_Timed.Enabled", initial: false, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Gruppe automatisch abschalten?", type: "boolean", def: false } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AutoOff_Timed.NoAutoOffWhenMotion", initial: true, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Kein AutoOff bei erkannter Bewegung?", type: "boolean", def: true } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".MotionSensors.Sensor0", initial: "", forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Datenpunkt des 1. Bewegungsmelders?", type: "string", def: "" } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AutoOn_Motion.Enabled", initial: false, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Gruppe bei Bewegung automatisch anschalten?", type: "boolean", def: false } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AutoOn_Motion.MinLux", initial: 100, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Mindesthelligkeit für AutoOn?", type: "number", unit: "Lux", def: 100 } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".LuxDp", initial: "", forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Datenpunkt des Helligkeitssensors?", type: "string", def: "" } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AdaptiveBri", initial: false, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Helligkeit mit Aussenhelligkeit steuern?", type: "boolean", def: false } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AdaptiveCt", initial: false, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Farbtemperatur durch Tageszeit steuern?", type: "boolean", def: false } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".RampOn.Enabled", initial: false, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Bei Anschalten Helligkeit langsam hochfahren?", type: "boolean", def: false } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".RampOff.Enabled", initial: false, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Bei Ausschalten Helligkeit langsam runterfahren?", type: "boolean", def: false } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".RampOn.Time", initial: 10, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Zeit für RampOn?", type: "number", unit: "s", def: 10 } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".RampOff.Time", initial: 10, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Zeit für RampOff?", type: "number", unit: "s", def: 10 } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".Blink.BlinkEnabled", initial: true, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Aktiviert Blinkfunktion", type: "boolean", def: true } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".Blink.BlinkNow", initial: false, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Löst kurzes blinken aus", type: "boolean", def: false } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".OnOverride", initial: false, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Putzlicht - Anschalten mit 100%, weiß, keinerlei Effekte", type: "boolean", def: false } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".RampOff.SwitchOutletsLast", initial: false, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Sollen Devices am Rampenende geschaltet werden?", type: "boolean", def: false } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AutoOn_Motion.AutoOnBri", initial: 0, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Helligkeit bei AutoOn", type: "number", unit: "%", def: 0 } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AutoOn_Motion.AutoOnColor", initial: "", forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Farbe bei AutoOn?", type: "string", def: "" } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".IsMotion", initial: false, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Summenstatus aller Bewegungsmelder", type: "boolean", def: false } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AutoOn_Lux.Enabled", initial: false, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Gruppe bei Dämmerung automatisch anschalten?", type: "boolean", def: false } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AutoOn_Lux.MinLux", initial: 100, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Mindesthelligkeit für AutoOn_Lux?", type: "number", unit: "Lux", def: 100 } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AutoOn_Lux.OnlyIfPresence", initial: false, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Nur ausführen wenn jemand anwesend ist?", type: "boolean", def: false } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AutoOn_PresenceIncrease.Enabled", initial: false, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Ausführen wenn jemand kommt", type: "boolean", def: false } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AutoOn_Lux.OnlyIfNoPresence", initial: false, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Nur ausführen wenn niemand anwesend ist?", type: "boolean", def: false } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AutoOn_PresenceIncrease.MinLux", initial: 60, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Mindesthelligkeit für AutoOn_PresenceIncrease", type: "number", unit: "Lux", def: 60 } };
    DpCount++;
    ...
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".RampOn.SwitchOutletsLast", initial: true, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Sollen Devices am Rampenende geschaltet werden?", type: "boolean", def: true } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AutoOn_Lux.AutoOnBri", initial: 0, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Helligkeit bei AutoOn?", type: "number", unit: "%", def: 0 } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AutoOn_Lux.AutoOnColor", initial: "", forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Farbe bei AutoOn?", type: "string", def: "" } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AutoOn_PresenceIncrease.AutoOnBri", initial: 0, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Helligkeit bei AutoOn", type: "number", unit: "%", def: 0 } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AutoOn_PresenceIncrease.AutoOnColor", initial: "", forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Farbe bei AutoOn?", type: "string", def: "" } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AutoOff_Lux.Enabled", initial: false, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Gruppe automatisch abschalten?", type: "boolean", def: false } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AutoOff_Lux.MinLux", initial: 100, forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Mindesthelligkeit für abschalten der Gruppe", type: "number", def: 0 } };
    DpCount++;
    ...
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AutoOff_Lux.Operator", initial: ">", forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Auf Über- oder Unterschreitung der Helligkeit prüfen?", type: "string", def: ">" } };
    DpCount++;
    States[DpCount] = { id: praefix + "." + x + ".AutoOn_Lux.Operator", initial: "<", forceCreation: false, common: { read: true, write: true, name: GroupNames[x] + " - " + "Auf Über- oder Unterschreitung der Helligkeit prüfen?", type: "string", def: "<" } };
    DpCount++;
...
function Init() {
    ...
    let z = 0;
    for (let x = 0; x < GroupNames.length; x++) { //Alle Gruppen durchgehen und Werte aller Dps in Group Array einlesen
        OldPowerState[x] = [];
        Groups[x] = []
        z = 0;
        Groups[x][50] = GroupNames[x]; //Den Namen  auf 50 
...
function DoPower(gruppe, onoff) { // onoff 0=aus, 1=an, 2=Gruppe um, 3=Devices um, 4=Save, 5=restore
    ...
    if (onoff == 1 && Groups[gruppe][0] == 1) { //Bei anschalten und wenn schon an
        if (logging) log("DoPower ignored duplicate on");
        return false;
    };
    ...
    if (onoff == 0) { // Gruppe ausschalten
        ...
        for (let x = 0; x < LightGroups[gruppe].length; x++) {
            ...
            if (x == LightGroups[gruppe].length - 1) {
                ...
                ClrTimeOut(gruppe); //Evtl vorhandenen Timeout löschen
                return true;
            };
        };
...
function DoCt(gruppe, ct = Groups[gruppe][2]) { //Farbtemperatur setzen
...
function DoBri(gruppe, bri = Groups[gruppe][1]) { //Helligkeit
...
function DoColor(gruppe, color = Groups[gruppe][3]) { //Farbe
    ...
    for (let x = 0; x < LightGroups[gruppe].length; x++) { //Alle Devices der Gruppe durchgehen
        if (typeof LightGroups[gruppe][x][3] != "undefined") { //Nachfolgendes nur ausführen wenn ein Arrayeintrag für Farbe vorhanden
            ...
            if (color.toString() == "255,255,255") { //Wenn weiss gefordert
                if (typeof LightGroups[gruppe][x][2] == "undefined" && typeof LightGroups[gruppe][x][3] !== "undefined") { //Device hat keine ct Funktion, aber color Funktion
                    if (Groups[gruppe][2] >= 50) { //Wenn ct > 50
                        if (typeof LightGroups[gruppe][x][3][3] !== "undefined") {
                            if (getState(LightGroups[gruppe][x][3][0]).val != LightGroups[gruppe][x][3][3]) setStateDelayed(LightGroups[gruppe][x][3][0], LightGroups[gruppe][x][3][3], 100); //Device auf Farbe, entsprechend warmweiss setzen  
                            if (logging) log("No ct function, but color function found, setting color to: " + LightGroups[gruppe][x][3][3] + ", what should be warmwhite" + " DeviceDp=" + LightGroups[gruppe][x][3][0])
                        };
...
function OnOverride(gruppe, onoff) {
    ...
    if (onoff == 1) {
        Groups[gruppe][1] = 100;//Helligkeit 100%
        Groups[gruppe][2] = 10;
        Groups[gruppe][3] = "white"; //Farbe auf weiss
    };
    Controller(gruppe, onoff)
...
function AutoOn_Motion(gruppe) { //Schaltet Gruppe bei Bewegung an. 
    ...
    if (Groups[gruppe][8] && UseLux <= Groups[gruppe][9] && !Groups[gruppe][0]) { //AutoOn_Motion ist enabled, Helligkeitstriggerwert wurde erreicht bzw. unterschritten und die Gruppe ist noch nicht an
        ...
        if (typeof Groups[gruppe][22] != "undefined" && Groups[gruppe][22] != "") { //33=.AutoOn_Motion.AutoOnColor ist nicht leer
            log("Set Color to " + Groups[gruppe][22])
            Groups[gruppe][3] = Groups[gruppe][22];
        } else {
            log("Set Color to " + Groups[gruppe][3])
            Groups[gruppe][3] = getState(praefix + "." + gruppe + ".Color").val; //Gruppencolor verwenden
        };
        ...
        if (typeof Groups[gruppe][21] != "undefined" && typeof Groups[gruppe][21] != "null" && Groups[gruppe][21] != "" && Groups[gruppe][21] != 0) { //32=.AutoOn_Motion.AutoOnBri ist nicht leer
            log("Set Bri to " + Groups[gruppe][21])

            Groups[gruppe][1] = Groups[gruppe][21];
        } else {
            if (Groups[gruppe][11]) {//Wenn Adaptive Bri ist aktiviert
                log("Set Bri to " + GetAdaptiveBri())
                Groups[gruppe][1] = GetAdaptiveBri(); //Adaptive bri Wert setzen
            } else {
                log("Set Bri to " + Groups[gruppe][1])
                Groups[gruppe][1] = getState(praefix + "." + gruppe + ".Bri").val; //Gruppenbri verwenden
            };
        };
        if (Groups[gruppe][12]) {//Wenn Adaptive Ct ist aktiviert
            log("Set Ct to " + GetAdaptiveCt())
            Groups[gruppe][2] = GetAdaptiveCt(); //Adaptive ct Wert setzen
        } else {
            log("Set Bri to " + Groups[gruppe][2])
            Groups[gruppe][2] = getState(praefix + "." + gruppe + ".Ct").val; //ct Wert aus Gruppe setzen
        };
            ...
            if (Groups[x][24] && ActualLux <= Groups[x][25] && !Groups[x][0] && !Groups[x][30]) { //Wenn .AutoOn_Lux.Enabled true und eingestellter Luxwert unterschritten ist und Gruppe ist inaktiv und Gruppe ist nicht gelockt
                ...
                if ((Groups[x][26] && PresenceCount > 0) || (Groups[x][28] && PresenceCount == 0)) { //Wenn ".AutoOn_Lux.OnlyIfPresence" ist true und Presence ist true ODER ".AutoOn_Lux.OnlyIfNoPresence" ist true und Presence ist 0
                    ...
                    if (typeof Groups[x][32] != "undefined" && typeof Groups[x][32] != "null" && Groups[x][32] != "" && Groups[x][32] != 0) { //32=.AutoOn_Lux.AutoOnBri ist nicht leer
                        Groups[x][1] = Groups[x][32];
                    } else {
                        if (Groups[x][11]) {//Wenn Adaptive Bri ist aktiviert
                            Groups[x][1] = GetAdaptiveBri(); //Adaptive bri Wert setzen
                        } else {
                            Groups[x][1] = getState(praefix + "." + x + ".Bri").val; //Gruppenbri verwenden
                        };
                    };
                    if (typeof Groups[x][33] != "undefined" && Groups[x][33] != "") { //33=.AutoOn_Lux.AutoOnColor ist nicht leer
                        Groups[x][3] = Groups[x][33];
                    } else {
                        Groups[x][3] = getState(praefix + "." + x + ".Color").val; //Gruppencolor verwenden
                    };
                    if (Groups[x][12]) {//Wenn Adaptive Ct ist aktiviert
                        Groups[x][2] = GetAdaptiveCt(); //Adaptive ct Wert setzen
                    } else {
                        Groups[x][2] = getState(praefix + "." + x + ".Ct").val; //ct Wert aus Gruppe setzen
                    };
                    Controller(x, 1);
                };
...
// 30=".AutoOn_Lux.DailyLock", 31= ".RampOn.SwitchOutletsLast", 32=".AutoOn_Lux.AutoOnBri", 33=".AutoOn_Lux.AutoOnColor", 34=".AutoOn_PresenceIncrease.AutoOnBri", 35=".AutoOn_PresenceIncrease.AutoOnColor"
async function Controller(gruppe, onoff) {
    log("Reaching Controller, Gruppe=" + gruppe);
    if (onoff == 0) { //Ausschalten
        Groups[gruppe][1] = 2; //Helligkeit auf 2 um Einschaltblitzer zu vermeiden

        if (await DoBri(gruppe, 2)) {
            DoPower(gruppe, 0);
            log("Done DoPower 0");
        }
    } else if (onoff == 1) {
        if (await DoPower(gruppe, 1)) { //Sobald alles angeschaltet
            log("Done DoPower 1");
            if (await DoColor(gruppe)) {
                log("Done DoColor");
                if (await DoBri(gruppe)) {
                    log("Done DoBri");
                    setTimeout(function () { //
...
function CreateTrigger() {
    ...
    for (let x = 0; x < Groups.length; x++) {
        on({ id: praefix + "." + x + ".Power", change: "ne" }, function (dp) { //Bei Statusänderung Power
            ...
            if (dp.state.val && !Groups[x][0]) {
                ...
                if (logging) log("Call Controller, values are: x=" + x + " Power=" + dp.state.val + " color=" + Groups[x][3])
                Controller(x, 1);
    ...
    on({ id: "zigbee.0.ccccccfffed4ee4c.color", change: "any" }, function (dp) { //Bei Statusänderung
